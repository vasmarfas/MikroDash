...
const NETWORKS = [
  net({ id: '*1', name: 'cap24', radio: 'cap24', ssid: 'LAGUNA', band: '2.4GHz',
  ...
  net({ id: '*3', name: 'cap5', radio: 'cap5', ssid: 'LAGUNA', band: '5GHz',
...
// ── 3. the SSID view is one row per network, aggregated ────────────────────
{
  ...
  assert.strictEqual(rows, 3,
    'expected one row per SSID (LAGUNA, Guest, Home), got ' + rows + ':\n' + html);
  // LAGUNA is on both bands and both must show, or the aggregate has silently
  // picked one.
